...
func _find_building_without_road_requirement() -> Resource:
	for data in grid_system.get_all_building_data().values():
		if not data.get("requires_road_adjacent"):
			return data
	return null
...
func test_plan_does_not_use_game_world_fallback() -> void:
	var zoning = FakeZoningSystem.new()
	var world = FakeGameWorld.new()
	world.zoning_system = zoning
	world.add_to_group("game_world")
	add_child(world)

	var building_data = _find_building_without_road_requirement()
	assert_not_null(building_data, "Expected a building with no road adjacency requirement.")

	var plan = grid_system.plan_building_placement(Vector2i(6, 6), building_data)
	assert_true(plan.can_place)
	assert_false(plan.reasons.has("FAR exceeded"))
